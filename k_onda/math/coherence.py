...
def fisher_z_from_coherence(Cxy_da: xr.DataArray) -> xr.DataArray:
    r = xr.apply_ufunc(np.abs, Cxy_da)                 # |coherence| in [0,1]
    r = xr.apply_ufunc(np.clip, r, 1e-12, 1 - 1e-12)   # avoid ±inf
    z = xr.apply_ufunc(np.arctanh, r)
    return z

def fisher_z_from_msc(msc_da: xr.DataArray) -> xr.DataArray:
    r = xr.apply_ufunc(np.sqrt, msc_da)                # sqrt coherence
    r = xr.apply_ufunc(np.clip, r, 1e-12, 1 - 1e-12)
    z = xr.apply_ufunc(np.arctanh, r)
    return z

def back_transform_fisher_z(z_da: xr.DataArray) -> xr.DataArray:
    r = xr.apply_ufunc(np.tanh, z_da)          # inverse of atanh
    r2 = xr.apply_ufunc(np.square, r)          # square it
    return r2
