...
class SelectMixin:
    ...
    def select_on_collection(self, collection, params):
        ...
        return types.Collection([member.select(**d, **extra) for member in collection])
    ...
    def select_on_signal(self, signal, params):
        ...
        if isinstance(selection, str):
            session = (
                signal.session if hasattr(signal, "session") else signal.origin.session
            )
            if selection == "epochs":
        ...
        if new_dim is not None and not isinstance(
            selection, (types.IntervalSet, types.MarkerSet)
        ):
            raise ValueError(
        ...
        if kwargs:
            dim_bounds, conditions = self.parse_select_kwargs(
                kwargs, selection, conditions
            )

        if dim_bounds:
            selection = self.locus_from_dim_bounds(
                signal, dim_bounds, units, conditions, params.metadim
            )

        if new_dim:
            if not metadim and not selection.metadim:
                raise ValueError(
                    "If you want to create a `new_dim` you must supply a `metadim`."
                )
