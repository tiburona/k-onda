...
def make_time_series(
    data, sampling_rate, start=0 * ureg.s, units="s", data_units=None, attrs=None
):
...
def make_data_series(data, dims, units, coords, data_units=None, attrs=None):
    ...
    da = da.assign_coords(
        {dim: getattr(coord, "magnitude", coord) for dim, coord in zip(dims, coords)}
    )

    da = da.pint.quantify({dim: unit for dim, unit in zip(dims, units)})

    if data_units is not None:
        da = da.pint.quantify(
            ureg.Unit(data_units) if isinstance(data_units, str) else data_units
        )
