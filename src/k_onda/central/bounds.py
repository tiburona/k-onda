...
class DimBounds(DictDelegator):
    ...
    def __init__(
        self, dim_pair_map=None, dim_pair_type="pad", units=None, metadim_of=None
    ):
    ...
    def _equivalent(self, key, other_key):
        if key == other_key:
            return True
        if key is None and other_key is None:
            return False
        return self._metadim_of and self._metadim_of(key) == self._metadim_of(other_key)

    def are_equivalent(self, my_dim, their_dim, strict):
        return (not strict and self._equivalent(my_dim, their_dim)) or (
            their_dim == my_dim
        )
