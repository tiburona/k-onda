    ...
    def data_schema(self):
        if self._data_schema is None:
            self._data_schema = self.transformer.make_output_schema(
                *[inp.data_schema for inp in self.inputs], 
...
class SignalStack(CalculateMixin, SignalMeanMixin, UnstackMixin):
    def __init__(
        ...
        schema_kwargs=None,
        stack_dim=None,
        stack_dim_was_added=None,
    ):
        inherited_stack_dim = None
        inherited_stack_dim_was_added = None
        if isinstance(inputs, type_registry.Collection):
            ...
            inherited_stack_dim = parent_stack.stack_dim
            inherited_stack_dim_was_added = parent_stack.stack_dim_was_added
        ...
        self.stack_dim = stack_dim or inherited_stack_dim or getattr(
            transformer, "dim", None
        )
        self.stack_dim_was_added = (
            stack_dim_was_added
            if stack_dim_was_added is not None
            else inherited_stack_dim_was_added
        )
        self.is_stack = True
    ...
    @property
    def parent(self):
        if getattr(self.inputs[0], "is_stack", False):
            return self.inputs[0]
        else:
